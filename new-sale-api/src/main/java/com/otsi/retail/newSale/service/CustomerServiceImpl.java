...
public class CustomerServiceImpl implements CustomerService {
	...
	public String saveUserData(UserDataVo vo) throws DuplicateRecordException {
		log.debug("deugging saveUserData:" + vo);
		if (vo.getPhoneNumber() != null) {
			Optional<UserData> user = userDataRepo.findByPhoneNumber(vo.getPhoneNumber());

			if (user.isPresent()) {
				log.error("User already exits with Mobile Number : " + user.get().getPhoneNumber());
				throw new DuplicateRecordException(
						"User already exits with Mobile Number : " + user.get().getPhoneNumber());
			}
		}
		...
		log.warn("we are testing if usre is saved...");
		log.info("after getting CustomerByMobileNumber :" + savedUser);
	...
	private void saveAVValues(UserDataVo vo, UserData savedUser) {
		log.debug("deugging saveAVValues()");
	...
	public UserDataVo getUserByMobileNo(Long mobileNum) throws RecordNotFoundException {
		log.debug("deugging getUserByMobileNo():" + mobileNum);
